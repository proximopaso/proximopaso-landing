...
function App() {
  ...
  const testimonials = [
    {
      text: "Gracias a Próximo Paso conseguí mi primer empleo IT desde el 4to ciclo universitario. ¡La mentoría es clave ganar experiencia real!",
      name: "María P.",
      role: "ESTUDIANTE UTP",
    ...
    {
      text: "Como estudiante de mitad de carrera, veía lejos el trabajo. Con Próximo Paso conseguí mi primera oportunidad laboral conectando directo con empresas.",
      name: "Carlos R.",
      role: "ESTUDIANTE UTP",
    ...
    <div style={{ width: '100%', minHeight: '100vh', background: 'var(--bg-light)', fontFamily: 'Lexend, sans-serif' }}>
      ...
      <nav style={{ width: '100%', padding: '16px 5%', background: 'var(--primary)', borderBottom: '1px var(--white-10) solid', display: 'flex', justifyContent: 'center', position: 'sticky', top: 0, zIndex: 101, backdropFilter: 'blur(6px)' }}>
        <div style={{ width: '100%', maxWidth: 1280, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} style={{ display: 'flex', alignItems: 'center', gap: 12, color: 'var(--bg-light)', fontSize: 24, fontWeight: '800', cursor: 'pointer' }}>
            <LogoIcon />
            Próximo Paso
          </div>

          <div className="nav-links">
            <span onClick={() => document.getElementById('why-us')?.scrollIntoView({ behavior: 'smooth' })} style={{ color: 'var(--bg-light-80)', fontSize: 14, fontWeight: '600', cursor: 'pointer' }}>¿Por qué nosotros?</span>
            <span onClick={() => document.getElementById('method')?.scrollIntoView({ behavior: 'smooth' })} style={{ color: 'var(--bg-light-80)', fontSize: 14, fontWeight: '600', cursor: 'pointer' }}>Método</span>
            <span onClick={() => document.getElementById('pricing')?.scrollIntoView({ behavior: 'smooth' })} style={{ color: 'var(--bg-light-80)', fontSize: 14, fontWeight: '600', cursor: 'pointer' }}>Precios</span>
          </div>

          <div className="nav-buttons" style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
            <div style={{ padding: '8px 16px', background: 'var(--white-10)', borderRadius: 8, color: 'var(--bg-light)', fontSize: 14, fontWeight: '700', display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
              <GlobeIcon /> ES
            </div>
            <div onClick={() => document.getElementById('contact-form')?.scrollIntoView({ behavior: 'smooth' })} style={{ padding: '10px 24px', background: 'var(--white)', borderRadius: 8, color: 'var(--primary)', fontSize: 14, fontWeight: '700', cursor: 'pointer' }}>Comunícate con nosotros</div>
          </div>
      ...
      <section className="section-padding" style={{ width: '100%', background: 'var(--primary)', padding: '96px 5%', position: 'relative', display: 'flex', justifyContent: 'center', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', bottom: 0, left: 0, width: '100%', height: '30%', background: 'linear-gradient(0deg, #094b5a 0%, transparent 100%)', zIndex: 1, pointerEvents: 'none' }} />
        ...
        <div className="hero-container" style={{ width: '100%', maxWidth: 1280, position: 'relative', zIndex: 2 }}>
          ...
          <div className="hero-text" style={{ display: 'flex', flexDirection: 'column', gap: 24, alignItems: 'flex-start' }}>
            <div style={{ padding: '6px 20px', background: 'var(--accent)', borderRadius: 999, color: 'var(--white)', fontSize: 12, fontWeight: '800', letterSpacing: 1.2, textTransform: 'uppercase' }}>IMPACTO NACIONAL</div>
            <h1 style={{ color: 'var(--white)', fontSize: 'clamp(48px, 6vw, 76px)', fontWeight: '900', lineHeight: '1.05', margin: 0 }}>
              Erradicando <br />el <span style={{ color: 'var(--accent)', textDecoration: 'underline' }}>Desempleo</span><br />Juvenil en el <br />Perú
            </h1>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, alignSelf: 'center', marginTop: '20px', zIndex: 10 }}>
            <button onClick={prevTestimonial} style={{ width: 40, height: 40, borderRadius: '50%', border: '1px solid rgba(255,255,255,0.3)', background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(8px)', color: 'white', display: 'flex', justifyContent: 'center', alignItems: 'center', cursor: 'pointer', flexShrink: 0 }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>
            </button>
            <div className="hero-card" style={{ padding: 32, background: 'rgba(255, 255, 255, 0.15)', borderRadius: 24, backdropFilter: 'blur(16px)', border: '1px solid rgba(255,255,255,0.2)', position: 'relative' }}>
              <div style={{ opacity: isTransitioning ? 0 : 1, transform: isTransitioning ? 'scale(0.96)' : 'scale(1)', transition: 'all 0.25s ease' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    ...
                    <div>
                      <div style={{ color: 'var(--white)', fontSize: 16, fontWeight: '800' }}>{testimonials[testimonialIndex].name}</div>
                      <div style={{ color: 'var(--accent)', fontSize: 11, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 0.5 }}>{testimonials[testimonialIndex].role}</div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <button onClick={nextTestimonial} style={{ width: 40, height: 40, borderRadius: '50%', border: '1px solid rgba(255,255,255,0.3)', background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(8px)', color: 'white', display: 'flex', justifyContent: 'center', alignItems: 'center', cursor: 'pointer', flexShrink: 0 }}>
      ...
      <section className="section-padding" style={{ width: '100%', background: 'var(--primary)', padding: '48px 5%', display: 'flex', justifyContent: 'center', borderTop: '1px solid var(--white-10)' }}>
        <div className="stats-grid" style={{ maxWidth: 1280 }}>
          {[
